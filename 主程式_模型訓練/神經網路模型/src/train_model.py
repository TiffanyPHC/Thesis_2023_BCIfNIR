...
def test_EEG_kernel(train_loader, test_loader, model, optimizer, epoch, learning_rate, weight_decay, wandb_import=False, rep_ID = ""):
    print("test_EEG_kernel--------------------------------------------------------------------------------------------------------------------------")
    rep_ID = str(rep_ID)
    ...
    for epoch in range(total_epoch):  # loop over the dataset multiple times
        ...
        if wandb_import:
            metrics = {
                "train_loss"+rep_ID: train_loss/train_total, 
                "train_accuracy"+rep_ID: 100 * train_correct / train_total,
                "test_loss"+rep_ID: test_loss/test_total, 
                "test_accuracy"+rep_ID: 100 * test_correct / test_total}
            wandb.log(metrics)
            if accfinal<(test_correct/ test_total) :
                accfinal=test_correct/ test_total
    
    if wandb_import:
        wandb.log({"best test Accuracy"+rep_ID: accfinal*100})
        #wandb.finish()
