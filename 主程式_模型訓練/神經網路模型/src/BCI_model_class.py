...
class fNIRSNet(nn.Module):
    def __init__(self,conv1_size=64, sep_conv_size=16 ,n_Hb=1, time_point=134, channel_num =20,  acti_fun='elu', pooling_type = 'avgPool', class_num = 3, con1_type = 'conv'):
        ...
        self.conv_sep = nn.Sequential(
            nn.Conv2d(self.n_Hb, self.F1, (1, self.conv1_size), padding=(0, self.padding), bias=False, groups=self.n_Hb),
            nn.Conv2d(self.F1, self.F1, (1, 1), bias=False),
            nn.BatchNorm2d(self.F1)
        )
    ...
    def temp_filter(self,x):
      if self.con1_type == 'conv':
        x = self.conv1(x)
        return x
      elif self.con1_type == 'sep':
        x = self.conv_sep(x)
        return x

    def forward(self, x):
        # x = self.conv1(x)
        x = self.temp_filter(x)
        x = self.conv2_1(x)
